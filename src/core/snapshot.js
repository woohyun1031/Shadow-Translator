...
const isBlock = (node) =>
    node && node.nodeType === Node.ELEMENT_NODE && structuralTags.has(node.tagName.toUpperCase());
 ...
 */
export function captureBlock(block, { assumeUntranslated = false } = {}) {
 ...
 */
export function captureAll(root) {
    ...
    root.querySelectorAll(blockSelector).forEach((block) => captureBlock(block, options));
}

export const getSnapshot = (block) => snapshots.get(block) || null;

export const getLastRendered = (block) => lastRendered.get(block);
export const setLastRendered = (block, translated) => lastRendered.set(block, translated);
export const clearLastRendered = (block) => lastRendered.delete(block);

// shadow를 일괄 제거했다면 렌더 기록도 함께 버려야 한다.
// 그러지 않으면 같은 번역문이 다시 들어올 때 "이미 렌더함"으로 판정해 shadow가 되살아나지 않는다.
export const resetLastRendered = () => {
